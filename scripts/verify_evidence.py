...
from rcfs_dq.verification import (
    VerificationError,
    assert_close,
    file_sha256,
    independent_regret,
...
def require(condition, message):
    """Do not use removable assertions for scientific or data validation."""
    if not condition:
...
def verify():
    ...
    require(
        file_sha256(manifest_path) == ORIGINAL_MANIFEST_SHA256,
        "Original scientific manifest changed",
    )
    ...
    for r in rows("configs/candidates.csv"):
        ...
        require(
            c.packed_bits == int(r["packed_bits"]) and c.saving_bits == int(r["saving_bits"]),
            "Candidate theoretical packed cost mismatch",
        )
        shapes = json.loads(r["shapes"])
        require(sum(int(np.prod(s)) for s in shapes.values()) == c.numel, "Shape/numel mismatch")
        require(c.cell_id not in candidates, "Duplicate candidate")
        candidates[c.cell_id] = c
    sets = defaultdict(list)
    for r in rows("configs/cost_sets.csv"):
        sets[r["set_id"]].append(candidates[r["cell_id"]])
    require(
        len(sets) == len(exact_cost_sets(list(candidates.values()))) == 27,
        "Exact-cost set count mismatch",
    )
    require(
        {tuple(sorted(c.cell_id for c in v)) for v in sets.values()}
        == {
            tuple(sorted(c.cell_id for c in v))
            for v in exact_cost_sets(list(candidates.values())).values()
        },
        "Exact-cost membership mismatch",
    )
    observations = defaultdict(dict)
    for r in rows("evidence/e1pred2r2/candidate_observations.csv"):
        key = int(r["class_id"]), int(r["seed"])
        require(r["cell_id"] not in observations[key], "Duplicate candidate observation")
    ...
    require(
        len(observations) == 64 and all(set(v) == set(candidates) for v in observations.values()),
        "Native observation coverage mismatch",
    )
    ...
    for (class_id, seed), records in sorted(observations.items()):
        for set_id, members in sorted(sets.items()):
            ...
            for policy, choice in selections.items():
                ...
                require(
                    result["resolved"] and result["oracle"] == reference["oracle"],
                    "Unresolved native risk or independent oracle mismatch",
                )
    ...
    require(len(classes) == 16, "Class coverage mismatch")
    means = {}
    for policy in selections:
        for metric in ["absolute_regret", "normalized_regret", "oracle_hit"]:
            ...
            for c in classes:
                ...
                require(len(units) == 108, "Class decision-unit coverage mismatch")
                per_seed = defaultdict(list)
                for seed, _, value in units:
                    per_seed[seed].append(value)
                require(
                    len(per_seed) == 4 and all(len(v) == 27 for v in per_seed.values()),
                    "Class seed/set coverage mismatch",
                )
    ...
    require(len(discovery) == 81, "Discovery cell coverage mismatch")
    require(
        all(r["cell_tangent_status"] == "DISCOVERY_TANGENT_SUPPORTED" for r in discovery),
        "Discovery tangent status mismatch",
    )
    for r in discovery:
        v = log_contrasts(
            float(r["LCG_actual"]),
    ...
    require(len(confirmation) == len(cells) == 9, "Confirmation sentinel coverage mismatch")
    require(
        sum(r["same_sign"] == "True" for r in confirmation) == 9,
        "Confirmation sign count mismatch",
    )
    require(
        sum(r["replicated"] == "True" for r in confirmation) == 8,
        "Confirmation strict replication count mismatch",
    )
    check(
    ...
    d = tail["detection"]
    require(
        tail["provisional_decision"] == "E1TAIL0_NO_USEFUL_EXCEPTION_SIGNAL",
        "Tail0 historical decision mismatch",
    )
    require(
        (d["TP"], d["FP"], d["FN"], d["TN"]) == (3, 474, 76, 1175),
        "Tail0 confusion matrix mismatch",
    )
